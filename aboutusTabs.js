const renderGroup = () => {
    ...
    .then(data => {
        ...
        data.map(value => {
            return(
                document.querySelector(".info").innerHTML += `<figure><img src="${value.src}" alt="${value.alt}"/></figure>`
            );
        })
...
const renderCompany = () => {
    ...
    .then(response => response.json())
    .then(data => {
        document.querySelector(".info").innerHTML =  `<figure><img src="img/aboutUs/vnet.jpg" alt="vnet"/></figure>` 
        document.querySelector(".info").innerHTML += "<article>";
        data.map(value => {
            return(
                document.querySelector(".info").innerHTML += `<h3>${value.title}</h3>
                <span>${value.p}<span>
